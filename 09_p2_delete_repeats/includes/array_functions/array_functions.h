...
T *allocate(int capacity = MINIMUM_CAPACITY)
{
    ...
    {
        return nullptr;        //if capacity is less than the minimum, return nullptr
                ...
                int many_to_copy)
{
    ...
    {
        *dest = *src;                        //copy the value pointed to by src to the location pointed to by dest.
        dest++;                              //increment dest to move to the right
        src++;                               //increment src as well
    }
}
...
T *copy_array(const T *src, int size)
{
    ...
    T *pointeR = ptr_arr;                    //pointer 'pointeR' keeps track of the start of the new array.
    for (int i = 0; i < size; i++)           //loops through the 'src' array to copy elements to 'ptr_arr'.
    {
        *ptr_arr = *src;                     //copies the value pointed to by 'src' to the location pointed to by 'ptr_arr'.
        ptr_arr++;                           //increment ptr_arr to move forward
        src++;                               //increment src as well
    }
    return pointeR;                          //returns a pointer to the newly created and populated 'ptr_arr'.
} 
