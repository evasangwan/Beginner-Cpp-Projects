...
int search(T *a, int size, const T &find_me)
{
    ...
    for (int i = 0; i < size; i++)        
    {
        if (*p == find_me)                //checks if value of a == find_me
        {
                ...
                int many_to_copy)
{
    ...
    {
        *destptr = *walker;                        //copy the value pointed to by walker to the location pointed to by dest.
        destptr++;                                 //increment dest to move to the right
...
T *copy_array(const T *src, int size)
{
    ...
    const T* ptr = src;                            //pointer ptr points to first element of src
    for (int i = 0; i < size; i++)           //loops through the 'src' array to copy elements to 'ptr_arr'.
    {
        *pointeR = *ptr;                     //copies the value pointed to by 'ptr' to the location pointed to by 'ptr_arr'.
        pointeR++;                           //increment ptr_arr to move forward
        ptr++;                               //increment ptr as well
    }
    return ptr_arr;                          //returns pointeR (points to the newly created and populated 'ptr_arr')
} 
