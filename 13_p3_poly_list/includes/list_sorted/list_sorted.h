...
class List{
    public:
        ...
        List(bool order = false, bool unique = true){
            _head_ptr = nullptr;
            _size = 0;
            _order = order;
            _unique = unique; 
        ...
        List(const List<T> &copy_this){
            ...
            _order = copy_this._order;
            _unique = copy_this._unique;
    ...
    node<T>* insert_after_(T i, node<T>* iMarker){
        node<T>*temp = _insert_after(_head_ptr,iMarker,i);
        _size++;
        return temp;
    }
    node<T>* insert_before_(T i, node<T>* iMarker){
        node<T>*temp =_insert_before(_head_ptr,iMarker,i);
        _size++;
        return temp;
    }
    node<T>* insert_sorted_(T i){
        node<T>*temp = _insert_sorted(_head_ptr,i,_order);
        if (!(_unique))
        {
            _size++;
        }
