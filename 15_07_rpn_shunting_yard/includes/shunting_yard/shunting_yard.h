...
//                 }
//                 else if (eq[i] == '-' && eq[i+1]== 'x' || eq[i+1]== 'X' && isdigit(eq[i+2])){   // -x^2 or -X^2 (unary minus)
//                     _postfix.push(new Function("unary"));
//                     // _postfix.push(new Operator("$"));

//                     // if (eq[i]== '-' && eq[i+1]== 'x' || eq[i+1] == 'X' && eq[i+2]== '^' && isdigit(eq[i+3])){
//                     //     if(!_postfix.empty()){
//                     //     _postfix.pop();
//                     //     }
//                     //     _postfix.push(new Operator("%"));
...
class ShuntingYard{
    ...
    ShuntingYard(const string& t):eq(t){
        ...
        for (int i = 0; i <eq.size(); i++){
            ...
            if (isdigit(eq[i]) || eq[i] == '.'){
                try{ if(i>1 && eq[i] == '0' && eq[i-1] == '/'){ //if something / by 0
                cout << "here ????? " << endl;
                    string a(eq[i], 1);
                    double num = stod(a);
                    throw double(num);
                }
                else if (i > 2 && eq[i-1] == '-' && eq[i-2] == '^'){  //if its x^-1 don't do anything since we took care of it in check for op
