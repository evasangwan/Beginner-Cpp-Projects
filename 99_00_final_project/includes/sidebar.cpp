...
Sidebar::Sidebar(float left, float width) : _left(left), _width(width)
{
    ...
    rect.setFillColor(sf::Color(204, 204, 255)); //(192,192,192)); //silver
    ...
    sb_text.setOutlineThickness(.5);
    sb_text.setOutlineColor(sf::Color(110,110,238));
    //sb_text.setFillColor(sf::Color(153,153,255));
    sb_text.setFillColor(sf::Color(110,110,238));
...
void Sidebar::draw(sf::RenderWindow &window)
{
    ...
    for (size_t i = 0; i < items.size(); i++){
        const string& equation = items[i];
        sb_text.setString(equation);
        sb_text.setPosition(sf::Vector2f(_left + LEFT_MARGIN, height));
        height += sb_text.getLocalBounds().height + VERTICAL_LINE_SPACING;
        //cout << "height of text : " << height << endl;
        if (i==0){
        _height=height;  //height of the firstbox
        }
        window.draw(sb_text);
    }
}
...
//added 
void Sidebar::_history(const vector<string> &h){
    items.clear();
